...
class Crawler():
    ...
    def gatherData(self, client):
        ...
        for feed in client.service.getFeeds(self.aid):
            if feed.assignmentStatus == 'accepted' and 'abonnement' not in feed.name.lower() and 'sim only' not in feed.name.lower() and 'simonly' not in feed.name.lower(): #Subscription feeds must be left out too.
                ...
                #If the previous url equals the current url, this is a nth file of the same company. Add a number to the end to make
    ...
    def save(self, campaignUrl, feedURL):
        self.log.info(str(time.asctime( time.localtime(time.time()) ))+": Saving:      C:/BorderSoftware/Boris/ProductFeeds/TradeTracker/" + campaignUrl + ".xml")
        
        #If the save failes, something is wrong with the file or directory name. Catch this error
        try:
            xmlFile = urllib.URLopener()
            xmlFile.retrieve(feedURL, "C:/BorderSoftware/Boris/ProductFeeds/TradeTracker/" + campaignUrl + ".xml")
            self.log.info(str(time.asctime( time.localtime(time.time()) ))+": Saved:       C:/BorderSoftware/Boris/ProductFeeds/TradeTracker/" + campaignUrl + ".xml")
        except:
            self.log.error(str(time.asctime( time.localtime(time.time()) ))+ ": " + traceback.format_exc())
            self.log.info(str(time.asctime( time.localtime(time.time()) ))+": Failed:       C:/BorderSoftware/Boris/ProductFeeds/TradeTracker/" + campaignUrl + ".xml")
